...
const UploadPage: React.FC = () => {
    ...
    const handleUpload = async () => {
        if (!file) {
            setMessage('Hãy chọn 1 file trước.');
            return;
        }
        ...
        try {
            ...
            if (response.status === 200) {
                setMessage('Link audio: '+ response.data.link);
                console.log(response);
            } else {
                setMessage('Tải file lên thất bại.');
            }
        } catch (error) {
    ...
    return (
        <div className="upload-container text-center">
            <h2>Hãy upload bài hát trước</h2>
            <label
                htmlFor="file"
                className="labelFile"
                onDrop={handleDrop}
